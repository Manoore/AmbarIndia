...
export default function MenuPage() {
  ...
  return <><div className="announcement">Order directly from Ambar India · Save on fees · Earn rewards</div><header className="site-header menu-page-header"><Brand /><nav><a href="/#story">Our story</a><a className="active" href="/menu">Menu</a><a href="/#catering">Catering</a><a href="/#visit">Visit us</a></nav><button className="outline-button" onClick={() => setCheckout(true)}>Your order ({cart.reduce((sum, item) => sum + item.quantity, 0)}) <b>→</b></button></header><main className="menu-page-main"><div className="menu-location-bar"><span>Ordering from <b>{selected.name}</b></span><select value={selectedId} onChange={(event) => { localStorage.setItem('ambar-selected-location', event.target.value); setSelectedId(event.target.value); }}>{locations.map((location) => <option value={location.id} key={location.id}>{location.name}</option>)}</select></div><MenuCatalogue onAdd={addToCart} /></main><footer><Brand /><div><a href="/">Home</a><a href="/#catering">Catering</a><a href="/#visit">Contact</a></div><p>© 2026 Ambar India Restaurant</p></footer>{checkout && <CheckoutDrawer location={selected} cart={cart} onClose={() => setCheckout(false)} onClear={() => setCart([])} onRemove={removeFromCart} />}</>;
}
